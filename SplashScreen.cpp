...
log4cxx::LoggerPtr CSplashThread::logger = log4cxx::Logger::getLogger("SplashScreen");
...
void CSplashThread::DestroySplash()
{
...
{
	LOG4CXX_INFO(logger, "Hiding splash screen");

	RECT rect;
	m_SplashScreen.GetWindowRect(&rect);
	m_SplashScreen.SetWindowPos(&CWnd::wndBottom, rect.left, rect.top,
		rect.right - rect.left,
		rect.bottom - rect.top, SWP_HIDEWINDOW);

	if (m_pNGEuserDlg != NULL)
	{
		m_pNGEuserDlg->EnableWindow(TRUE);
	}
}

void CSplashThread::ShowSplash()
{
	LOG4CXX_INFO(logger, "Showing splash screen");

	if (m_pNGEuserDlg != NULL)
	{
		m_pNGEuserDlg->EnableWindow(FALSE);
	}

	RECT rect;
	m_SplashScreen.GetWindowRect(&rect);
	m_SplashScreen.SetWindowPos(&CWnd::wndTopMost, rect.left, rect.top,
		rect.right - rect.left,
		rect.bottom - rect.top, SWP_SHOWWINDOW);
}

int CSplashThread::ShowMessageBox(LPCSTR lpText, LPCSTR lpCaption, UINT uType)
{
	return MessageBoxA(m_SplashScreen, lpText, lpCaption, uType);
}

void CSplashThread::SetNGEuserDlg(CNGEuserDlg *pNGEuserDlg)
{
	m_pNGEuserDlg = pNGEuserDlg;
	m_SplashScreen.SetNGEuserDlg(pNGEuserDlg);
}
...
void CSplashThread::SetText(CString text)
{
	LOG4CXX_INFO(logger, "Setting text to: " << (CStringA)text);
	m_SplashScreen.SetText(text);
}
...
{     
  ...
  else
  {
	  ...
	  CPngImage pngImage;
	  if (pngImage.Load(m_pszResourceName))
	  {
		  hBitmap = (HBITMAP)pngImage.Detach();
	  }
	  else
	  {
		  HINSTANCE hinstRes = AfxFindResourceHandle(m_pszResourceName, RT_BITMAP);
		  if (hinstRes == NULL)
		  {
			  return NULL;
		  }
		  ...
		  hBitmap = (HBITMAP) ::LoadImage(hinstRes, m_pszResourceName, IMAGE_BITMAP, 0, 0, uiLoadImageFlags);
	  }
  }
...
void CSplashWnd::OnPaint()
{
  ...
  CRect rcText = CRect(0, m_nHeight - TEXT_HEIGHT, m_nWidth, m_nHeight);
  rcText.DeflateRect(20, 20);
  ...
  CFont font;
  font.CreateFont(TEXT_HEIGHT / 2, 0, 0, 0, 300, FALSE, FALSE, FALSE, (BYTE)0, (BYTE)3,
	  (BYTE)2, (BYTE)1, DEFAULT_PITCH,
	  NULL);
  CFont *pOldFont = (CFont*)dc.SelectObject(&font);
  dc.SetTextColor(RGB(0, 0, 0));
  dc.DrawText(m_sText, rcText, DT_CENTER | DT_SINGLELINE | DT_VCENTER);
...
void CSplashWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == 27) //ESC key
	{
		int retVal = AfxMessageBox(_T("Exit the application?"), MB_OKCANCEL | MB_TOPMOST);
		if (retVal == IDOK)
		{
			m_pNGEuserDlg->Close();
		}
...
void CSplashWnd::SetNGEuserDlg(CNGEuserDlg *pNGEuserDlg)
{
	m_pNGEuserDlg = pNGEuserDlg;
}
...
void CSplashWnd::SetText(CString text)
{
	m_sText = text;
	if (m_hWnd != NULL)
		Invalidate();
