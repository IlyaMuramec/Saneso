...
void CThinPlateSpline::setSourceSize(const Size2f& pS)
{
	inputSize = pS;
}
...
Point2f CThinPlateSpline::interpolate_forward_(const Point& p)
{
	Point2f interP;
...
Point2f CThinPlateSpline::interpolate_back_(const Point& p)
{
	Point2f interP;
...
Point2f CThinPlateSpline::interpolate(const Point& p, const TPS_INTERPOLATION tpsInter)
{
...
void CThinPlateSpline::warpImage(const Mat& src, Mat& dst, float lambda, const int interpolation,const TPS_INTERPOLATION tpsInter)
{
	...
	Mat imgROI = dst(outputRoi);
	remap(src, imgROI, mapx, mapy, interpolation);
}
...
void CThinPlateSpline::computeMaps(const Size& dstSize, Mat& mx, Mat& my, const TPS_INTERPOLATION tpsInter)
{
	Point p(0, 0);
	Point_<float> intP(0, 0);
	outputRoi = Rect(Point(0, 0), dstSize);
	...
	if (tpsInter == BACK_WARP)
	{
		...
		for (int row = 0; row < dstSize.height; row++) {
			for (int col = 0; col < dstSize.width; col++) {
				...
				if ((intP.x >= 0) && (intP.x < inputSize.width)
					&& (intP.y >= 0) && (intP.y < inputSize.height))
				{
		...
		outputRoi = Rect(minLeft, minTop, maxRight - minLeft, maxBottom - minTop);
	}
#endif

#if 1
	Mat_<float> mxFlt = Mat_<float>(outputRoi.size());
	Mat_<float> myFlt = Mat_<float>(outputRoi.size());

	for (int row = 0; row < outputRoi.height; row++) {
		for (int col = 0; col < outputRoi.width; col++) {
			p = Point(col + outputRoi.x, row + outputRoi.y);
			intP = interpolate(p, tpsInter);
			mxFlt(row, col) = intP.x;
			myFlt(row, col) = intP.y;
		}
	}

	// Convert to int for faster operation
	cv::convertMaps(mxFlt, myFlt, mx, my, CV_16SC2);
#else
	mx = Mat_<float>(outputRoi.size());
	my = Mat_<float>(outputRoi.size());

	for (int row = 0; row < outputRoi.height; row++) {
		for (int col = 0; col < outputRoi.width; col++) {
			p = Point(col + outputRoi.x, row + outputRoi.y);
			intP = interpolate(p, tpsInter);
			mx.at<float>(row, col) = intP.x;
			my.at<float>(row, col) = intP.y;
		}
	}
#endif
