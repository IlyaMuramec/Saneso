...
int CMicroController::Connect()
{
	...
	for (int i = 0; i < retryCount; i++)
	{
		...
		CSerial::EnumerateSerialPorts(comPortNumbers);
	...
	{
		MessageBox(NULL, _T("Unable to connect to MicroController.\nPlease check USB Cable and verify Processor box is powered on."), _T("Error"), MB_OK);

		/*int ret = MessageBox(NULL, _T("Simulate micro controller?"), _T("Error"), MB_YESNO);
		if (ret == IDYES)
		{
			SimulateControlMessages();
		}*/
	}
}
...
DWORD CMicroController::SendMesage(const void* lpSendBuf, DWORD dwSendCount, void* lpRecvBuf, DWORD dwRecvCount, bool bLog = true)
{
	...
	try
	{
		if (_bConnected)
		{
			...
			void *ptr = &buffer[0];
			m_pSerialPort->SendData((const char*)ptr, buffer.size());
...
UINT CMicroController::ReceiveLoop()
{
	...
	while (!_bExiting)
	{
		...
		while (_bConnected)
		{
			...
			try
			{
				bytesRead = m_pSerialPort->ReadData(&lpRecvBuf, 1);
			}
			catch (...)
			{
				...
				if (lpRecvBuf == START_END_CHAR)
				{
					...
					if (bAync || (pMsg->type == GET_STATUS_RESPONSE))
					{
						if (pMsg->type == MC_CONTROL_CHANGED)
						{
							...
							LOG4CXX_DEBUG(logger, "Handset message received Control = " << std::hex << control
								<< " Value = " << std::hex << value);
						}
...
UINT CMicroController::SimulateLoop()
{
	...
	while (!_bExiting)
	{
		...
		int control = pMsg->msgData[0];
		int value = pMsg->msgData[1];
		LOG4CXX_DEBUG(logger, "Handset simulation message received Control = " << std::hex << control
				<< " Value = " << std::hex << value);
...
int CMicroController::ReadVersion()
{
	...
	m_pSerialPort->SendData((const char*)&buffer[0], buffer.size());

	byte msgRecv[128];
	DWORD bytesRead = m_pSerialPort->ReadData(&msgRecv, 128);
	LOG4CXX_INFO(logger, "Read bytes " << bytesRead);
...
int CMicroController::ReadStatus()
{
	...
	byte msgRecv[128];
	DWORD bytesRead = SendMesage(&msgSend[0], 4, &msgRecv[0], 8, false);
...
int CMicroController::SetWhiteBalanceGains(int board, float red, float green, float blue)
{
	...
	LOG4CXX_INFO(logger, "Setting RGB gains to " << red << " " << green << " " << blue
		<< " on board " << board);
...
int CMicroController::WriteCameraRegister(int board, unsigned int reg, unsigned short value)
{
	int retVal = -1;
	...
	LOG4CXX_DEBUG(logger, "Writing register " << std::hex << std::showbase << reg
		<< " to " << value << " on board " << std::dec << board);
...
int CMicroController::SendCameraInit(int board)
{
	int retVal = -1;
	...
	{
		retVal = WriteCameraRegister(board, 0x3820, 0x18);
	}
...
int CMicroController::SetLEDValue(int channel, float value)
{
	...
	byte msgRecv[128];
	DWORD bytesRead = SendMesage(&msgSend[0], 6, &msgRecv[0], 0, false);
...
int CMicroController::SetLEDOnOff(bool front, bool on)
{
	int retVal = -1;

	LOG4CXX_INFO(logger, "Setting " << (front ? "Front" : "Side") << " leds to " << (on ? "On" : "Off"));
	...
	{
		LOG4CXX_INFO(logger, (front ? "Front" : "Side") << " leds were turned " << (on ? "On" : "Off"));
		retVal = 0;
	}
...
void CMicroController::SetCameraResetLine(bool set)
{
	...
	{
		LOG4CXX_INFO(logger, "Reset line was set to " << (set ? "HIGH" : "LOW"));
	}
}
...
int CMicroController::ReadEEPromMemoryLocation(EEPromLocation location, unsigned int offset, std::vector<byte> &bytes)
{
	...
	LOG4CXX_INFO(logger, "Reading  " << (location == ProcessorBox ? "Box" : "Scope") << " EEProm at location 0x"
	...
	if ((msgRecv[0] == START_END_CHAR) && (msgRecv[1] == READ_EEPROM_RESP))
	{
		...
		if (len = bytes.size())
		{
			...
			LOG4CXX_INFO(logger, "ReadEEProm  for location 0x" 
